...
object AdventureGUI extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    def updateInfo(info: String) = {
      val loc = game.player.location
      if (!this.game.isOver) {
        this.turnOutput.text = info
      } else {
        this.turnOutput.text = info + "\n\n" + this.game.goodbyeMessage
      }
      if(loc.name == "Corridoor")
        if (game.player.attributes("wisdom")< 2) this.locationInfo.text = loc.fullDescription /** + "\nThe staircase to the west looks promising!" */
        else this.locationInfo.text = this.player.location.fullDescription + "\nYou notice that there's something wrong with the staircase to the west."
      else if(loc.name == "Pit" && player.lastLocation.name == "Corridoor") {
        game.player.isDead = true
        this.locationInfo.text = /**loc.fullDescription + */ "The stairs have been rigged! It's a trap!\nAs you start your descent the steps fold in and you fall down into a deep pit!\nThe fall damage is too much. Better wise up next time. Game Over."
      } else this.locationInfo.text = loc.fullDescription
